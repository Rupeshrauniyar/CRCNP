...
export default function SignupPage() {
  ...
  const onSignup = async (e) => {
    ...
    if (!/^[a-z0-9_]{3,20}$/.test(normalizedUsername)) {
      setError("Username must be 3-20 characters and use only lowercase letters, numbers, or underscore.");
      return;
    }
      ...
      <div className="mx-auto w-full max-w-[1200px]">
        <div className="mb-8">
          ...
          <h1 className="text-[clamp(1.75rem,3.5vw,2.5rem)] font-extrabold tracking-[-0.025em] text-[#1C1917] leading-[1.1]">
            Join CRCNP in seconds
          </h1>
          <p className="mt-3 text-base text-[#6B6560] leading-[1.65] max-w-[520px]">
            Create your account and start reporting issues that matter in your area.
          </p>
        </div>
        ...
        <div className="w-full max-w-xl rounded-3xl border border-black/10 bg-white p-6 md:p-8 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          ...
          {message && (
            <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-700">
              {message}
            </div>
          )}
          ...
          <div className="mt-5 text-center text-sm text-[#6B6560]">
            ...
            <Link href="/signin" className="font-semibold text-[#1C1917] underline-offset-4 hover:underline">
              Sign in
            </Link>
          </div>
        </div>
